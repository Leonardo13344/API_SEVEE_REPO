<template>
    <v-container fluid class="contenedor">
        ...
        <v-card flat color="c-Inicio" width="100%">
          ...
          <v-window touchless v-model="onboarding">
            <v-window-item :value="0">
              <div >
                <MapaExtranjero v-bind:expand_b = this.boolean1   @add="(i) => numero_vuelta = i"></MapaExtranjero>
              </div>
            </v-window-item>
      ...
      <v-row no-gutters gutters  class="margen" v-if="numero_vuelta == 0">
        <InfoGeneral />
      </v-row>
...
export default {
  ...
  components: { InfoGeneral, InfoGeneral2, CodGraficos, CodGraficos2, CodTablas, MapaExtranjero }
