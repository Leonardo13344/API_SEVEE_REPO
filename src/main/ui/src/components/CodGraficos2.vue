<template>
    <v-row no-gutters justify="space-around">
      ...
      <v-card
      :class="`loader${condicionEstilo()}0`">
        <div :class="`loader${condicionEstilo()}1`">
          <GChart
            type="ColumnChart"
            :data="chartData"
            :options=opcionesNormal()
          />
        </div>
        ...
        <div :class="`loader${condicionEstilo()}2`">
          <GChart
            type="ColumnChart"
            :data="chartDataSombra"
            :options=opcionesSombra()
          />
        </div>
        
        <div :class="`img_presidente${condicionEstilo()}`"> 
            <img  
              :class="`espaciado_img_presidente${condicionEstilo()}`"
              v-for="(item) in arrayImgPresidente()"
              :key="item.id"
              :src="item.url"
              :alt="item.name"
            />
        ...
        <div :class="`img_partido${condicionEstilo()}`"> 
          <div :class="`espaciado_partido${condicionEstilo()}`"
            
            v-for="(item) in arrayImgPartidoPolitico()"
            :key="item.id">
            <img  
              :class="`altura_${Math.trunc(((info[item.id-1] * 100) / Math.max(...info))/10) }`"
              :src="item.url"
              :alt="item.name"
            />
          </div>
        ...
        <div :class="`txt${condicionEstilo()}`" >
          <h1  
            :class="`espaciado_txt${condicionEstilo()}`"
    ...
    methods:{
      ...
      opcionesNormal(){
        this.chartOptions = {
          ...
          bar: { groupWidth: "25%" },
          vAxis: { gridlines: { count: 0, color: 'transparent' }, textStyle: {fontSize: 1}, viewWindow: 'maximized', 
